...
type Handlers = {
  onToggleLeader?: (memberId: string, isLeader: boolean) => Promise<void>
  onRemove?: (memberId: string) => Promise<void>
}

export function getDepartmentMembersColumns(
  churchId: string,
  handlers?: Handlers
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {handlers?.onToggleLeader && !isLeader && (
                <DropdownMenuItem
                  onClick={() => handlers.onToggleLeader!(row.original.id, true)}
              ...
              {handlers?.onToggleLeader && isLeader && (
                <DropdownMenuItem
                  onClick={() => handlers.onToggleLeader!(row.original.id, false)}
                  className="flex items-center gap-2 cursor-pointer"
                >
                  <Star className="h-4 w-4" />
                  Remove Leader
              ...
              {handlers?.onRemove && (
                <DropdownMenuItem
                  onClick={() => handlers.onRemove!(row.original.id)}
