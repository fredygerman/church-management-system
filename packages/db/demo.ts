...
async function run() {
  ...
  await db.insert(offerings).values(memberRows.slice(0, 4).map((member, i) => ({ churchId: id.church, categoryId: id.tithe, memberId: member.id, sessionId: id.session, amountCents: (i + 1) * 250000, currency: "TZS", offeringDate: "2026-08-30", showOnDonorWall: i === 0 }))).onConflictDoNothing()
